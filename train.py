...
def train(config):
	...
	train_dataset = dataloader.lowlight_loader(config.lowlight_images_path, config.train_input_size)		
	...
	for epoch in range(config.num_epochs):
		for iteration, img_lowlight in enumerate(train_loader):
			...
			if ((iteration+1) % config.snapshot_iter) == 0:
				...
				wandb.save(config.snapshots_folder + "Epoch" + str(epoch) + '.pth')		
...
if __name__ == "__main__":
	...
	parser.add_argument('--train_input_size', type=int, default=512)
